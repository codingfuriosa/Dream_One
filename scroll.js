...
(() => {
  ...
  if (hlIsMobile && hlCards.length > 1) {
    ...
    hlCards.forEach((card, i) => {
      ...
      // Dim the receding card with brightness (NOT opacity). Element opacity
      // makes the whole card — background included — translucent, so the card
      // sliding over it bleeds through and the text looks washed out. Brightness
      // keeps the card fully opaque while still letting it visually recede.
      gsap.fromTo(card,
  ...
  if (cfgIsMobile) {
    ...
    cfgCards.forEach((card, i) => {
      ...
      // Dim with brightness, not opacity — see HIGHLIGHTS note above. Element
      // opacity made the receding card translucent so the covering card's text
      // was hard to read; brightness keeps every card fully opaque.
      gsap.fromTo(card,
        { scale: 1, filter: 'brightness(1)' },
        {
          scale: 0.94,
  ...
  gsap.utils.toArray('[data-plan]').forEach((card, i) => {
    // NOTE: pure 2D lift-in (no rotationY / perspective). A 3D-transformed card
    // becomes a 3D rendering context, which makes Chrome mis-composite the
    // child img's blur() filter and smear it across the whole card/section.
    gsap.fromTo(card,
      { opacity: 0, y: 70, transformOrigin: '50% 100%' },
      {
        opacity: 1, y: 0,
        duration: 1.1, ease: 'expo.out',
        delay: (i % 3) * 0.08,
        // Strip the inline transform once the lift-in ends so nothing lingers
        // on the card to keep corrupting the child blur.
